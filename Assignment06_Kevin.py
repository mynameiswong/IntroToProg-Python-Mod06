...
dicRow = {}                     # A row of data separated into elements of a dict {Task,Priority}
...
class IO:
    ...
    def AddDataToList(list_of_rows):
        """ Ask user input for Task and Priority
    ...
    def RemoveDataFromList(list_of_rows):
        """ Ask user input for Task to delete from list. Then update update user Task was deleted or not.
...
def main():
    ...
    while(True):
        IO.OutputMenuItems()                # Displays menu option
        ...
        # Step 3.2 - Add a new item to the list/Table
            ...
            IO.AddDataToList(lstTable)          # Ask user for new Task and Priority and add item to the List/Table
            IO.ShowCurrentItemsInList(lstTable) # Show current data in the list/table
            continue                            # to show the menu
    
        # Step 3.3 - Remove a new item to the list/Table
        elif(strChoice == '3'):
            IO.RemoveDataFromList(lstTable)     # Ask user for item to delete and confirm to delete item from list
            IO.ShowCurrentItemsInList(lstTable) # Show current data in the list/table
            continue  # to show the menu
        ...
        # Step 3.4 - Save tasks to the ToDoFile.txt file
            ...
            IO.ShowCurrentItemsInList(lstTable)  # Show current data in the list/table
            FileProcessor.WriteListDataToFile(strFileName, lstTable)   # Ask the user if they want to save that data
            continue
    
        # Step 3.5 - Reload data from the ToDoFile.txt file (clears the current data from the list/table)
        elif (strChoice == '5'):
            FileProcessor.ReloadDataFromFile(strFileName, lstTable)
            continue  # to show the menu
